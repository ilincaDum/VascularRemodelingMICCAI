...
4. Stage 2 - rescue matching for leftovers (mutual nearest neighbour)
5. Damage detection
"""
...
def node_lobe(G, n):
    d   = G.nodes[str(n)]
    lab = _first_attr(d, ["lobe", "Lobe", "label_lobe", "lobe_label", "region_lobe"],
                      None, cast=str)
    if lab is None:
        return ""
    s = str(lab)
...
def _agg_damage_df(df):
    ...
    return df.groupby(["bl_u", "bl_v"], as_index=False).agg({
        "damaged": "max",
        "damage_features": lambda s: ",".join(
            sorted(set([t for x in s.fillna("") for t in x.split(",") if t]))
        ),
    })


# ---------------------------------------------------------------------------
# Umeyama global alignment
# ---------------------------------------------------------------------------
...
def _lsa_feature_cost(G_bl, G_fu, bl_pair, fu_pair):
    ...
    vals = [v for v in (_rel(Lb, Lf), _rel(Rb, Rf)) if np.isfinite(v)]
...
def _fast_lsa_by_label_featured(C_fu, C_bl, pairs_fu, pairs_bl, label,
                                  G_BL_local, G_FU_local):
    empty_prov = pd.DataFrame()
    if C_fu.shape[0] == 0 or C_bl.shape[0] == 0:
        return [], [], {
            "lobe": label, "gate_mm": None,
            "FU_edges": C_fu.shape[0], "BL_edges": C_bl.shape[0], "matched": 0,
        }, empty_prov
    ...
    if not cand_map:
        return [], list(range(C_fu.shape[0])), {
            "lobe": label, "gate_mm": gate,
            "FU_edges": C_fu.shape[0], "BL_edges": C_bl.shape[0], "matched": 0,
        }, empty_prov
    ...
    for i, hits in cand_map.items():
        ...
        for hh, d in zip(hits, dists):
            ...
            if USE_FEATURED_LSA and FEAT_COST_WEIGHT > 0:
                total = ((1.0 - FEAT_COST_WEIGHT) * (d / (gate + 1e-6))
                         + FEAT_COST_WEIGHT * _lsa_feature_cost(
    ...
    gate_row = {
        "lobe": label, "gate_mm": float(gate),
        "FU_edges": int(C_fu.shape[0]), "BL_edges": int(C_bl.shape[0]),
        "matched": int(len(rows)),
    }
    return rows, fu_unmatched, gate_row, pd.DataFrame(prov_rows)


# ---------------------------------------------------------------------------
# Stage 2 — Mutual nearest-neighbour rescue
# ---------------------------------------------------------------------------

def rescue_with_mutual_nn(bl_leftover_set, fu_leftover_set, bl_xyz, fu_xyz,
                           G_BL_local, G_FU_local,
                           radius_mm, length_tol_frac, mutual=True):
    if not bl_leftover_set or not fu_leftover_set:
        return [], [
            {"lobe": _edge_label_sanitized(G_BL_local, u, v), "bl_u": u, "bl_v": v}
            for (u, v) in bl_leftover_set
        ], pd.DataFrame()

    fu_list      = list(fu_leftover_set)
    fu_centroids = np.vstack([(fu_xyz[u] + fu_xyz[v]) * FAST_DTYPE(0.5) for (u, v) in fu_list])
    fu_lengths   = np.asarray([edge_feat(G_FU_local, u, v, "length") for (u, v) in fu_list],
                               dtype=FAST_DTYPE)
    ...
    bl_lengths   = np.asarray([edge_feat(G_BL_local, u, v, "length") for (u, v) in bl_list],
                               dtype=FAST_DTYPE)
    ...
    for i, (u, v) in enumerate(bl_list):
        ...
        if not cand:
            disappeared.append({"lobe": _edge_label_sanitized(G_BL_local, u, v),
                                 "bl_u": u, "bl_v": v})
            continue

        dists = np.linalg.norm(fu_centroids[cand] - bl_c, axis=1)
        rel   = (np.abs(fu_lengths[cand] - bl_L) / (abs(bl_L) + 1e-6)
                 if bl_L != 0 else np.full(len(cand), np.inf))
        cost  = (RESCUE_COST_WEIGHT_DIST * (dists / (radius_mm + 1e-6))
                 + RESCUE_COST_WEIGHT_FEAT * rel)
        ...
        if dists[jloc] < radius_mm and rel[jloc] < length_tol_frac:
            ...
            prov_rows.append({
                ...
                "match_confidence": float(np.clip(
                    1.0 - (0.6 * (dists[jloc] / (radius_mm + 1e-6))
                           + 0.4 * rel[jloc]), 0.0, 1.0)),
                "centroid_dist_mm": float(dists[jloc]),
    ...
    return rescued, disappeared, pd.DataFrame(prov_rows)
...
def compute_damage_flags(G_BL, G_FU, survived_df, drop_frac):
    ...
    if survived_df is None or survived_df.empty:
        return pd.DataFrame(columns=["lobe", "bl_u", "bl_v", "fu_u", "fu_v",
                                      "damaged", "damage_features"])
    ...
    for r in survived_df.itertuples(index=False):
        ...
        for f in DMG_FEATURES:
            ...
            if not (np.isfinite(bl) and np.isfinite(fu)) or bl == 0:
                continue
            rel = (fu - bl) / bl
            if ((DMG_MODE == "decrease_only" and rel <= -drop_frac)
                    or (DMG_MODE != "decrease_only" and abs(rel) >= drop_frac)):
                triggers.append(f)
...
def per_lobe_counts(survived, disappeared, damaged_df):
    ...
    s   = (survived.groupby("lobe").size().rename("survived")
           if not survived.empty else pd.Series(dtype=int, name="survived"))
    d   = (disappeared.groupby("lobe").size().rename("disappeared")
           if not disappeared.empty else pd.Series(dtype=int, name="disappeared"))
    out = (pd.concat([s, d], axis=1).fillna(0).astype(int)
           .reset_index().rename(columns={"index": "lobe"}))
    ...
    if damaged_df is not None and not damaged_df.empty:
        ...
        if not k.empty:
            ...
            feat_lists = (k.groupby("lobe")["damage_features"]
                           .apply(_join_feats).rename("damaged_features_used"))
...
def write_labeled_graphs_single_label(G_BL, G_FU, survived, disappeared,
                                       damaged_df, pair_dir: Path):
    ...
    survived_c    = (_canon_pair_df(survived,    "bl_u", "bl_v")
                     if survived    is not None else pd.DataFrame())
    disappeared_c = (_canon_pair_df(disappeared, "bl_u", "bl_v")
                     if disappeared is not None else pd.DataFrame())
    damaged_c     = (_agg_damage_df(damaged_df)
                     if (damaged_df is not None and not damaged_df.empty)
                     else pd.DataFrame(columns=["bl_u", "bl_v", "damaged", "damage_features"]))

    disappeared_set = (set(map(tuple, disappeared_c[["bl_u", "bl_v"]].astype(str).values))
                       if not disappeared_c.empty else set())
    damaged_set     = (set(map(tuple, damaged_c[damaged_c["damaged"] == True][["bl_u", "bl_v"]]
                               .astype(str).values))
                       if not damaged_c.empty else set())
    survived_set    = (set(map(tuple, survived_c[["bl_u", "bl_v"]].astype(str).values))
                       if not survived_c.empty else set())

    for (u, v) in _edge_tuples_sorted(H_BL.edges()):
        if   (u, v) in disappeared_set: label = "disappeared"
        elif (u, v) in damaged_set:     label = "damaged"
        elif (u, v) in survived_set:    label = "survived"
        else:                           label = "disappeared"
...
def _model_rows_for_pair(G_BL, pair_dir: Path, survived_df, disappeared_df,
                          damage_df, agg_csv_path: Path):
    survived_df = (_canon_pair_df(
        _ensure_cols(survived_df, {"bl_u": "", "bl_v": "", "fu_u": "", "fu_v": ""}),
        "bl_u", "bl_v") if survived_df is not None else pd.DataFrame())
    disappeared_df = (_canon_pair_df(
        _ensure_cols(disappeared_df, {"bl_u": "", "bl_v": ""}),
        "bl_u", "bl_v") if disappeared_df is not None else pd.DataFrame())
    damage_df = (_agg_damage_df(
        _ensure_cols(damage_df, {"bl_u": "", "bl_v": "", "damaged": False,
                                  "damage_features": ""}))
        if damage_df is not None else pd.DataFrame())

    bl_all      = _edge_tuples_sorted(G_BL.edges())
    bl_all_rows = pd.DataFrame([{"bl_u": u, "bl_v": v} for (u, v) in bl_all])

    surv_mark = (survived_df.assign(_survived=1)[["bl_u", "bl_v", "fu_u", "fu_v", "_survived"]]
                 if not survived_df.empty
                 else pd.DataFrame(columns=["bl_u", "bl_v", "fu_u", "fu_v", "_survived"]))
    bl_all_rows = bl_all_rows.merge(surv_mark, how="left", on=["bl_u", "bl_v"])

    disc_mark = (disappeared_df.assign(_disappeared=1)[["bl_u", "bl_v", "_disappeared"]]
                 if not disappeared_df.empty
                 else pd.DataFrame(columns=["bl_u", "bl_v", "_disappeared"]))
    ...
    if not damage_df.empty:
        bl_all_rows = bl_all_rows.merge(
            damage_df[["bl_u", "bl_v", "damaged", "damage_features"]],
            how="left", on=["bl_u", "bl_v"])
    else:
        bl_all_rows["damaged"]         = False
        bl_all_rows["damage_features"] = ""

    bl_all_rows.loc[bl_all_rows["_survived"] != 1,
                    ["damaged", "damage_features"]] = [False, ""]
    ...
    bl_all_rows["label_damaged"]  = (bl_all_rows["final_label_bl"]
                                     .isin(["disappeared", "damaged"]).astype(int))
    ...
    bl_all_rows.to_csv(pair_dir / "edges_bl_fu_modeling_rows.csv", index=False)

    agg_csv_path.parent.mkdir(parents=True, exist_ok=True)
    header_needed = not agg_csv_path.exists()
    bl_all_rows.to_csv(agg_csv_path,
...
def _plot_overlay(G_BL, G_FU, out_png: Path):
    ...
    for u, v, d in G_BL.edges(data=True):
        ...
        if   lab == "survived":    _seg(ax, xyz(G_BL,u), xyz(G_BL,v), lw=1.0, a=0.90, color="#1f77b4")
        elif lab == "disappeared": _seg(ax, xyz(G_BL,u), xyz(G_BL,v), lw=1.6, a=0.85, color="#d62728")
        elif lab == "damaged":     _seg(ax, xyz(G_BL,u), xyz(G_BL,v), lw=2.2, a=0.95, color="#7f3c8d")
...
def _pair_report_row(pair_dir: Path, drop_frac: float,
                      survived_df, disappeared_df, dmg_df, prov_df,
                      n_bl_edges: int, n_fu_edges: int):
    parts                        = pair_dir.parts
    patient, modality, timepoint = parts[-3], parts[-2], parts[-1]

    if prov_df is None or prov_df.empty or "match_confidence" not in prov_df.columns:
        match_frac = 0.0
        mean_conf = med_conf = mean_dist = med_dist = mean_gate = np.nan
    else:
    ...
    n_dmg  = (int((dmg_df["damaged"] == True).sum())
              if (dmg_df is not None and "damaged" in dmg_df.columns) else 0)
...
def run_pair(BL_path: Path, FU_path: Path, save_pair_dir: Path,
             agg_csv_path: Path, report_csv_path: Path,
             drop_frac: float, show_inline: bool = False):
    ...
    def _build_side(G, xyz_map):
        ...
        cents = (np.vstack([(xyz_map[u] + xyz_map[v]) * FAST_DTYPE(0.5)
                             for (u, v) in pairs])
                 if len(pairs) else np.zeros((0, 3), dtype=FAST_DTYPE))
        by_label = {}
    ...
    if not BL["pairs"] or not FU["pairs"]:
        ...
        disappeared_df = pd.DataFrame([
            {"lobe": _edge_label_sanitized(G_BL, u, v), "bl_u": u, "bl_v": v}
            for (u, v) in _edge_tuples_sorted(BL["pairs"])
        ])
        dmg_df  = pd.DataFrame(columns=["lobe", "bl_u", "bl_v", "fu_u", "fu_v",
                                          "damaged", "damage_features"])
        ...
        per_lobe_counts(survived_df, disappeared_df, dmg_df).to_csv(
            save_pair_dir / "change_counts_by_lobe.csv", index=False)
        write_labeled_graphs_single_label(G_BL, G_FU, survived_df, disappeared_df,
                                          dmg_df, save_pair_dir)
        _model_rows_for_pair(G_BL, save_pair_dir, survived_df, disappeared_df,
                              dmg_df, agg_csv_path)
        prov_df.to_csv(save_pair_dir / "match_provenance.csv", index=False)
        _append_report_row(report_csv_path,
                           _pair_report_row(save_pair_dir, drop_frac,
                                            survived_df, disappeared_df, dmg_df, prov_df,
                                            n_bl_edges, n_fu_edges))
        try:
    ...
    rows_c, _, _, prov_c = _fast_lsa_by_label_featured(
        C_fu, C_bl,
        [FU["pairs"][i] for i in idx_fu],
        [BL["pairs"][i] for i in idx_bl],
        "Central", G_BL, G_FU)
    survived_rows.extend(rows_c)
    if not prov_c.empty:
        prov_rows.append(prov_c)

    lobes = sorted({k for k in (set(BL["idx_by_label"]) | set(FU["idx_by_label"]))
                    if k != "Central"})
    for L in lobes:
        ...
        C_bl   = BL["centroids"][idx_bl] if len(idx_bl) else np.zeros((0, 3), dtype=FAST_DTYPE)
        C_fu   = FU["centroids"][idx_fu] if len(idx_fu) else np.zeros((0, 3), dtype=FAST_DTYPE)
        rows_L, _, _, prov_L = _fast_lsa_by_label_featured(
            C_fu, C_bl,
            [FU["pairs"][i] for i in idx_fu],
            [BL["pairs"][i] for i in idx_bl],
            L, G_BL, G_FU)
    ...
    bl_survived_set = (_edge_tuples_sorted(survived_df[["bl_u", "bl_v"]].values)
                       if not survived_df.empty else set())
    bl_leftover_set = _edge_tuples_sorted(BL["pairs"]) - bl_survived_set

    fu_survived_set = (_edge_tuples_sorted(survived_df[["fu_u", "fu_v"]].values)
                       if not survived_df.empty else set())
    ...
    rescued_rows, disappeared_rows, prov_res = rescue_with_mutual_nn(
        ...
        radius_mm=HEAVY_RESCUE_RADIUS,
        length_tol_frac=HEAVY_LENGTH_TOL,
    ...
    if prov_res is not None and not prov_res.empty:
        prov_rows.append(prov_res)

    disappeared_df = (pd.DataFrame(disappeared_rows)[["lobe", "bl_u", "bl_v"]]
                      if disappeared_rows
                      else pd.DataFrame(columns=["lobe", "bl_u", "bl_v"]))

    survived_df    = _ensure_cols(_canon_pair_df(survived_df,    "bl_u", "bl_v"),
                                  {"lobe": "", "bl_u": "", "bl_v": "", "fu_u": "", "fu_v": ""})
    disappeared_df = _ensure_cols(_canon_pair_df(disappeared_df, "bl_u", "bl_v"),
                                  {"lobe": "", "bl_u": "", "bl_v": ""})
    ...
    dmg_df = _ensure_cols(dmg_df, {"lobe": "", "bl_u": "", "bl_v": "",
                                    "fu_u": "", "fu_v": "", "damaged": False,
                                    "damage_features": ""})
    dmg_df.to_csv(save_pair_dir / "matched_edges_damage_flags.csv", index=False)
    ...
    prov_df = pd.concat(prov_rows, ignore_index=True) if prov_rows else pd.DataFrame()
    prov_df.to_csv(save_pair_dir / "match_provenance.csv", index=False)
...
def discover_pairs(input_root: Path, valid_modalities):
    ...
    for pat_dir in sorted([p for p in input_root.glob("P*/") if p.is_dir()]):
        for mod_dir in sorted([m for m in pat_dir.iterdir()
                                if m.is_dir() and m.name in set(valid_modalities)]):
            bl = mod_dir / "BASELINE" / "sanitize" / "central_sanitized.graphml"
            if not bl.exists():
                continue
            for tp_dir in sorted([t for t in mod_dir.iterdir()
                                   if t.is_dir() and t.name.upper().startswith("FU")]):
...
def run_all_thresholds(input_root: Path, save_root_base: Path,
                        thresholds, valid_modalities=None,
    ...
    for drop_frac in thresholds:
        ...
        for k, (pid, mod, tp, bl, fu) in enumerate(pairs, 1):
            ...
            try:
                run_pair(BL_path=bl, FU_path=fu,
                ...
                s = int(counts["survived"].sum())    if "survived"    in counts.columns else 0
                d = int(counts["disappeared"].sum()) if "disappeared" in counts.columns else 0
                g = int(counts["damaged"].sum())     if "damaged"     in counts.columns else 0
...
def _parse_args():
    p = argparse.ArgumentParser(
        description="Baseline↔Follow-up vessel graph matching + labeling "
                    "(LSA + mutual-NN rescue + damage flags).")
    p.add_argument("--input-root",  type=str, required=True,
                   help="Root directory containing P*/(Artery|Vein)/... structure.")
    p.add_argument("--save-root",   type=str, required=True,
                   help="Base output directory; subfolders per threshold are created.")
    p.add_argument("--thresholds",  type=float, nargs="+", default=THRESHOLDS_DEFAULT,
                   help="Damage drop thresholds, e.g. 0.5 0.75 0.9")
    p.add_argument("--modalities",  type=str,   nargs="+", default=VALID_MODALITIES_DEFAULT,
                   help="Modalities to process, e.g. Artery Vein")
    p.add_argument("--seed",        type=int,   default=None,
                   help="Optional RNG seed (affects PASS0 subsampling).")
    p.add_argument("--show-inline-first-n", type=int, default=SHOW_INLINE_FIRST_N_DEFAULT,
                   help="Kept for CLI compatibility; plots are always saved to disk.")
    return p.parse_args()
