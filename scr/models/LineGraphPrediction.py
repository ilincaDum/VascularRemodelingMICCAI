...
this script:
- loads labeled graphml pairs per damage thresholdand vessel modality
- applies matching quality check 
- builds a cached, capped line-graph representation per pair once
- runs lopo cv with shared patientwise folds and trains two model configurations (FiLM and Ablation)
- writes fold-level metrics, summary metrics, and dose-related analyses:
  - dose_damage_trend.csv (+ correlation)
...
for k in edge_num_keys_with_dose + edge_bool_keys + edge_cat_keys:
    if _is_leaky_key(k):
...
def _as_bool01(x):
    if isinstance(x, (bool, np.bool_)): return 1.0 if x else 0.0
    s = str(x).strip().lower()
...
def materialize_pair(rec, cfg,
    ...
    l_oh = np.zeros((len(l_ids), len(lobe_vocab)), np.float32); l_oh[np.arange(len(l_ids)), l_ids] = 1.0
    s_oh = np.zeros((len(s_ids), len(side_vocab)), np.float32); s_oh[np.arange(len(s_ids)), s_ids] = 1.0
    parts += [l_oh, s_oh]
    ...
    if T is None:
        return (Xt, yt, edge_index, mp_mask_t, None, None)
...
class ResidualGatedLineGraphGNN_FlexibleFiLM(nn.Module):
    def __init__(self, d_in, hid=128, dropout=0.15, rounds=2, film_scale=0.10, t_dim=0):
        super().__init__()
    ...
    def forward(self, X, edge_index, mp_mask, T=None, Tok=None):
        E = X.size(0)
...
def collate_pairs(pair_tensors, keys):
    ...
    return X, y, edge_index, mp, T, Tok
...
def eval_gnn(model, cache, keys, pair_tensors, store_preds=False):
    ...
    P = np.concatenate(Ps); Y = np.concatenate(Ys); PAT = np.concatenate(Pats)
...
def run_all_configs_for_thr_mod(thr: str, modality: str):
    ...
    log(f"\n[thr={thr} | mod={modality}] pairs pre-qc: {len(pairs)} | patients={pairs['patient'].nunique()}")

    pairs = apply_qc(pairs)
    log(f"[thr={thr} | mod={modality}] pairs post-qc: {len(pairs)} | patients={pairs['patient'].nunique()}")
    ...
    trend_out = outdir_base / "dose_damage_trend.csv"
    ...
    if trend is not None:
        ...
        safe_to_csv(pd.DataFrame([{"corr_bin_median_dose_vs_damage_rate": corr}]),
    ...
    for cfg in configs:
        ...
        for fold in folds:
            ...
            time_list = [cache[pk]["fuidx_raw"] for pk in train_keys]
            mu_time, sd_time = fit_norm_1d(time_list)
            dose_list = [cache[pk]["dose_raw"] for pk in train_keys]
            mu_dose, sd_dose = fit_norm_1d(dose_list)
            ...
            if cfg["film_mode"] == "none":
                t_dim = 0
            elif cfg["film_mode"] == "dose":
                t_dim = 1
            else:
                t_dim = 2
            ...
            rows.append(dict(
                ...
                macro_auc=ma, macro_ap=mp, micro_auc=mia, micro_ap=mip
            ))
            preds_store.append((PAT, Y, P, DOSE))

            log(f"[{thr}|{modality}|{cfg_name}] rep={rep:02d} test={','.join(sorted(test_p))} "
        ...
        out_cal = outdir / f"dose_calibration_{cfg_name}.csv"
        g = dose_calibration_from_predictions(PAT, Y, P, DOSE, out_cal, bin_edges=bin_edges)
        if g is None:
            log(f"[{cfg_name}] dose calibration skipped (insufficient preds)")
        else:
            log(f"[{cfg_name}] dose calibration saved -> {out_cal}")

        log(f"[{cfg_name}] saved -> {outdir/'folds.csv'} {outdir/'summary.csv'}")

    log(f"\n[{thr}|{modality}] done. folds -> {outdir_base/'cv_folds_patientwise_SHARED.csv'}")


def main():
    set_seed(seed)
    log(f"device={device} | amp={use_amp and device=='cuda'} | cap_lg_in_deg={cap_lg_in_deg} | film_scale={film_scale}")
    ...
    log("configs:")
    for c in configs:
        log(f"  - {c['name']}: x_use_dose={c['x_use_dose']} x_use_time={c['x_use_time']} film_mode={c['film_mode']}")
